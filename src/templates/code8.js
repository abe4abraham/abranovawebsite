...
function mk_responsive_nav() {
  ...
  $('.mk-nav-responsive-link').on(eventtype, $.debounce( 500, true, function( e ) {
    var $body = $('body'),
...
function mk_milestone() {
  ...
  if ($.exists('.mk-milestone')) {
    ...
    $('.mk-milestone'+play).each(function () {
      ...
      var num_to_str = function(number) {
      	var number_str = number.toString(),
      		text_number = '';
      	for(var i = 0; i < number_str.length; i++){
      		if (i >0 && (number_str.length - i)  % 3 == 0) text_number += ',';
      		text_number += number_str[i];
      	}
     		return text_number;
      }
      ...
      if (!$(this).hasClass('scroll-animated')) {
        ...
        $({
          ...
          easing: 'linear',
          step: function () {
            el_this.find('.milestone-number').text(num_to_str(Math.floor(this.countNum)));
          },
          complete: function () {
            el_this.find('.milestone-number').text(num_to_str(this.countNum));
          }
        });
      }
    });
...
$('a[href^="#"][data-animate]').on('click', function(e) {
  e.preventDefault();
